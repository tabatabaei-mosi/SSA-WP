...
class SSA_WP:
    ...
    def map_backward(
        ...
        for idx in locs_id:
            ...
            final_idx = final_idx_check(
                idx, len_obj_map=len(self.best_rqi_locs))
    ...
    def npv_calculator(self, capex: float = 0) -> float:
        ...
        for i in range(start, final_index + 1):
            ...
            npv = npv + (((FOPT_i * r_o) - (FWPT_i * r_wp) -
                          (FWIT_i * r_wi)) / (1.01 ** counter))
    ...
    def obj_func(self, solution: np.ndarray) -> float:
        ...
        _, space_faults = self.loc_penalty(
            locs,
            target='min_space',
            n_optwell=self.t_opt_well,
            min_well_space=self.well_space,
            obj_func=True
        )
