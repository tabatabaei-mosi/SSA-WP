...
def rock_index(
    ...
    indices = np.where((rock_quality_index >= threshold) & (rock_quality_index <= rock_quality_index.max()))
...
def clean_interval(
    ...
    for idx, loc in enumerate(rqi_loc_list):
        ...
        rqi_s, rqi_c, rqi_e = rqi_matrix[x, y, z_s], rqi_matrix[x, y, z_c], rqi_matrix[x, y, z_e]
...
def filter_locs(
    ...
    Tuple[List[float], List[Tuple[int, int, int]], List[Tuple[int, int, int]], List[float], List[Tuple[int, int, int]]],
    ...
    if check_interval:
        ...
        final_rqi_locs_, final_rqi_, _, _ = clean_interval(final_rqi_locs, final_rqi, rqi_matrix, TH=threshold)
...
def main(
    ...
    # Filter locations
    best_rqi, best_rqi_locs, null_blocks, rqi_interval, locs_interval = filter_locs(grid_path, prop_key, null_space, rqi_limit, dim, check_interval=True)
