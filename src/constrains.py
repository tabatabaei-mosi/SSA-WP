    ...
    if target == 'min_space':
        for well_number, well_i in enumerate(well_item):
            for next_index in range(well_number + 1, n_optwell):
                ...
                distance = np.sqrt(np.square((well_i[0] - well_j[0])) + np.square((well_i[1] - well_j[1])))
                if distance < min_well_space:
                    result.append([f'> well {well_number + 1} & {next_index + 1}', f' distance = {round(distance, 3)}'])
                    fault += 1
        if not result:
    ...
    if target == 'border':
        for well_num, well in enumerate(well_item, start=1):
            ...
            if loc_i in (1, grid_size[0]) or loc_j in (1, grid_size[1]):
                result.append(f'> Well {well_num}, is on the border --> i = {loc_i}, j = {loc_j}')
                fault += 1
        if not result:
...
def null_block_penalty(
    ...
    well_locs: List[Union[float, int]] = None,
    well_perfs: List[Union[float, int]] = None,
    opt_well_number: int = 10,
    min_null_space: int = 2,
    opt_solution: List[Union[float, int]] = None,
    obj_func: bool = False
) -> Tuple[List[str], int]:
    """
    ...
    if obj_func is False:
        locs_id, prod_rate, inj_rate = split_solution(opt_solution)
        locs = map_backward(locs_id, best_rqi_locs)
        well_locs, well_perfs = decode_locs(locs)
    
    locs_well = [list(well_locs[j:j + 2]) for j in range(0, opt_well_number * 2, 2)]
    perf_well = [list(well_perfs[j:j + 2]) for j in range(0, opt_well_number * 2, 2)]
    ...
    for well_number in range(opt_well_number):
        ...
        for null in null_block_list:
            ...
            dis_sq_c = np.square(loc_i - null_i) + np.square(loc_j - null_j) + np.square(perf_c)
            dis_c = np.sqrt(dis_sq_c)
            dis_sq_s = np.square(loc_i - null_i) + np.square(loc_j - null_j) + np.square(perf_s)
            dis_s = np.sqrt(dis_sq_s)
            dis_sq_e = np.square(loc_i - null_i) + np.square(loc_j - null_j) + np.square(perf_e)
